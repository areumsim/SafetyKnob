...
│   ├── analyze_caution_predictions.py # Caution 예측 분석
│   ├── compare_experiments.py         # Scenario vs Caution-Excluded 비교
│   ├── visualize_tsne.py              # t-SNE 임베딩 시각화
│   └── visualize_tsne_cached.py       # 캐싱된 임베딩 t-SNE 시각화
│
├── experiments/           # 🧪 실험 스크립트
│   ├── train_from_embeddings.py  # 캐싱 임베딩 기반 probe 학습 (주요)
│   ├── train_standalone.py       # 독립형 학습 (임베딩 추출 포함)
...
│   ├── SO-35/ ~ SO-47/           # 시나리오별 원본 데이터
│   ├── safe/, danger/, caution/  # 간단한 분류 테스트용
│   ├── test_all/                 # 통합 테스트 데이터
│   └── cache/                    # Embedding 캐시
│
├── results/               # 📊 실험 결과 및 분석 보고서
│   ├── scenario/                 # Scenario (Random) split 결과
│   ├── temporal/                 # Temporal split 결과
│   ├── single_models/            # 단일 모델 결과
│   ├── multiseed/                # Multi-seed 검증 결과 (87실험 × 5시드)
│   ├── ensemble_ablation/        # 앙상블 조합 ablation 결과
│   ├── dann/                     # DANN domain adaptation 결과
│   ├── scaling_curve/            # 데이터 스케일링 곡선
│   ├── temporal_per_category/    # 카테고리별 temporal 분석
│   ├── caution_excluded/         # Caution 제외 실험 결과
│   ├── multitask/                # Multi-task 학습 결과
│   ├── statistical_tests/        # 통계적 유의성 검정 결과 (JSON)
│   ├── figures/                  # 논문용 Figure 9개 (PNG)
...
│   ├── RESEARCH_REPORT_FINAL.md  # 최종 연구 보고서
│   └── FINAL_REPORT.md           # 최종 결과 요약
│
├── reports/               # 📈 분석 보고서 (⚠️ Directory to be created)
│   ├── RESEARCH_ANALYSIS.md      # 연구 방향 분석
│   ├── experiment_results_YYYYMMDD.json
│   └── figures/                  # 시각화 그래프
│
├── checkpoints/           # 💾 모델 체크포인트 (⚠️ Directory to be created)
│   ├── baselines/
│   ├── single_models/
│   └── ensemble/
│

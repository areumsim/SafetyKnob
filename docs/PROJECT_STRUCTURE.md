...
│
├── scripts/               # 🔧 유틸리티 스크립트
│   ├── benchmark_v2.py           # v2 벤치마크 스크립트
│   ├── prepare_dataset.py        # ⚠️ Planned: 데이터 전처리
│   ├── verify_dataset.py         # ⚠️ Planned: 데이터 무결성 검증
│   ├── verify_environment.py     # ⚠️ Planned: 환경 검증
│   ├── aggregate_results.py      # ⚠️ Planned: 실험 결과 집계
│   └── visualize_embeddings.py   # ⚠️ Planned: Embedding 시각화
│
├── experiments/           # 🧪 실험 스크립트 (⚠️ Directory Planned)
│   ├── train_single_model.py     # 단일 모델 훈련
│   ├── evaluate_model.py         # 모델 평가
│   ├── train_ensemble.py         # 앙상블 훈련
│   ├── train_baselines.py        # Baseline 모델 훈련
│   └── configs/                  # 실험 설정 파일
│
├── data/                  # 💾 데이터셋
│   ├── raw/                      # AI Hub 원본 데이터
...
│   └── cache/                    # Embedding 캐시
│
├── results/               # 📊 실험 결과 (⚠️ Directory to be created)
│   ├── baselines/                # Baseline 모델 결과
│   ├── single_models/            # 단일 모델 결과
│   ├── ensemble/                 # 앙상블 결과
│   └── summary_YYYYMMDD.json     # 전체 요약
│

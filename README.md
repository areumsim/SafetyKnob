...
**핵심 연구 질문**:
1. 사전학습 모델의 표현이 경량 probe로 산업 안전 분류에 충분한가? (→ 2-layer MLP로 96.13% F1 달성, 단 true linear probe는 80.43%로 비선형 변환 필수)
2. 다중 모델 앙상블이 단일 모델보다 분포 변화(distribution shift)에 강건한가? (→ **실패**: 앙상블 < 최고 단일 모델, temporal shift 28-35%p 하락)
3. 카테고리별 독립 분류기가 multi-task 대비 우수한가? (→ **부정적 전이 확인**: 5개 카테고리 중 4개에서 독립 학습이 우수. 1이미지=1라벨 구조로 인해 multi-task 학습이 구조적으로 불리)
...
**핵심 발견**:
- SigLIP + 2-layer MLP: F1 96.13%, AUC 99.37%
- True linear probe: F1 80.43% (비선형 변환 필수, "선형 분리" 주장 수정)
- SigLIP-Frozen(656K params) > ResNet50-Finetuned(24.6M params): Foundation model 우위 입증
- 경계 사례에 대한 81.2% unsafe 편향 (보수적 판단)

**상세 결과**: [results/FINAL_REPORT.md](results/FINAL_REPORT.md)

#### Stage 2: 카테고리별 독립 분류 및 강건성 검증 ✅ **완료** (2026-03-24)

**목표**: 카테고리별 독립 분류 성능 및 앙상블/temporal 강건성 검증

**완료된 실험**:
- ✅ 5개 카테고리 독립 학습 (Fall, Collision, Equipment, Environment, PPE)
- ✅ Independent vs Multi-task 비교 → 독립 학습 우위 확인 (4/5 카테고리)
- ✅ Ensemble 방법론 (Weighted Voting) → 실패 (앙상블 < 최고 단일 모델)
- ✅ Temporal distribution shift 검증 → 치명적 하락 (28-35%p)
- ✅ Probe depth ablation (Linear/1-layer/2-layer) → 비선형 변환 필수
- ✅ ResNet50-Frozen baseline → Foundation model 우위 입증 (+17.71%p)

**핵심 발견**: 현재 라벨 구조(1이미지=1카테고리)는 multi-task 학습에 부적합. "카테고리별 독립 분류"가 정확한 표현.
...
- **카테고리별 위험 분류**: 5가지 독립적 위험 카테고리(Fall/Collision/Equipment/Environment/PPE) 분석으로 해석 가능성 향상
...
#### Probe Depth Ablation (RQ1 — Scenario_v2 Test Set, 2,203장, 5-seed mean±std)

> **주의**: 아래 결과는 sequence-level split (`data_scenario_v2/`) 기준. 이전 frame-level split 결과는 데이터 누출로 무효화됨.
> 상세: `results/CRITICAL_FINDING_SCENARIO_LEAKAGE.md`

| 모델 | Linear F1 | 1-Layer F1 | 2-Layer F1 | 3-Layer F1 |
|------|-----------|------------|------------|------------|
...
> **RQ1 판정**: Linear→1-layer gap = 10.35%p → 비선형 변환 1회 필수.
> **1-layer ≈ 2-layer** (gap 0.12%p) → 1개 hidden layer면 충분.
> 3-layer는 과적합으로 역효과. **최적 아키텍처: 1-layer MLP (591K params)**.

#### Baseline 비교

| 모델 | Mode | Trainable Params | Test F1 | 비고 |
|------|------|-----------------|---------|------|
| **ResNet50** | Frozen | 1,049,601 | 78.42% | ImageNet representation (old split) |
| **SigLIP** | Frozen (1-layer) | 591,361 | **87.16±0.26%** | Foundation model + probe (scenario_v2) |

> ResNet50 baseline은 old split 기준이므로 scenario_v2에서 재실험 필요.
> Foundation model의 경량 probe 활용 가능성은 유효하나, 절대 성능은 87% 수준.

#### Data Scaling Curve (SigLIP 2-layer, scenario_v2, 5-seed mean±std)

| 학습 데이터 비율 | N_train | Test F1 |
|-----------------|---------|---------|
| 10% | ~1,012 | 74.04±0.59% |
| 25% | ~2,530 | 82.23±1.41% |
| 50% | ~5,060 | 85.86±0.46% |
| 75% | ~7,589 | 86.46±0.97% |
| 100% | 10,119 | 87.28±0.45% |

> 25%만으로도 82.23% F1 달성. 50% 이상에서 수확체감 (50→100% gain: 1.42%p).

#### Temporal Shift 완화 시도 (SigLIP, 5-seed mean��std)

| 방법 | Temporal F1 | vs Baseline | 비고 |
|------|-----------|-------------|------|
| Baseline (frozen 2-layer) | 66.11±0.72% | - | - |
| DANN (clean split) | 65.28±0.88% | -0.83%p | frozen embedding에서 무효 |
| Category reweighting | 66.50±0.69% | +0.39%p | label shift 아님 |
| **LoRA fine-tuning (r=16)** | **77.77±1.02%** | **+11.59%p** | gap의 38.7% 회복 |

> **Temporal shift는 feature-level distribution change** (조명/날씨/계절 변화). Frozen embedding 기반 방법(DANN, label shift)은 무효.
> LoRA가 backbone 수정을 통해 38.7% 회복하지만 완전 해소는 미달성. 상세: `results/TEMPORAL_SHIFT_ANALYSIS.md`

### 성능 지표 설명

- **Val F1 Score**: 전체 안전/위험 분류 정확도 (95% = 100장 중 95장 정확)
- **Val AUC**: 분류기의 본질적 성능 (99% = 거의 완벽한 분류 능력)
- **Avg Dim F1**: 5개 위험 카테고리(낙상/충돌/장비/환경/보호구) 평균 정확도 (카테고리별 독립 분류기 기준)
...
- **Labels**: 5개 카테고리 안전 라벨 (Fall, Collision, Equipment, Environment, PPE) — 1이미지당 1개 활성 카테고리
...
- **치명적 발견**:
  - Val F1 95.49% → Test F1 **66.11±0.72%** (🚨 **-30.0%p 하락**, 5-seed)
  - **모든 Foundation Models 취약**: CLIP -30.8%p, DINOv2 -34.3%p
  - **원인**: 조명/날씨/계절 변화, 카메라 각도 변경, 행동 패턴 변화
- **완화 시도**: DANN(효과 없음), LoRA fine-tuning(+11.59%p, 38.7% 회��)
- **결과 위치**: `results/temporal_clean/`, `results/lora_temporal/`, `results/dann_clean/`
- **결론**: Temporal shift는 feature-level 변화로, frozen embedding 방법으론 해결 불가. Backbone 수정(LoRA)이 부분 효과.
...
| `experiments/train_from_embeddings.py` | 사전추출 임베딩 기반 경량 probe 학습 (~3초) | `python experiments/train_from_embeddings.py --model siglip --embeddings-dir embeddings/scenario/siglip --probe-depth linear` |
| `scripts/extract_embeddings.py` | Foundation model 임베딩 사전추출 및 캐싱 | `python scripts/extract_embeddings.py --model siglip --data-dir data_scenario --output embeddings/scenario/siglip` |
...
- **[results/STATISTICAL_ANALYSIS.md](results/STATISTICAL_ANALYSIS.md)** - 통계적 유의성 분석 (paired t-test, McNemar, bootstrap CI, Bonferroni 보정)
- **[results/TEMPORAL_SHIFT_ANALYSIS.md](results/TEMPORAL_SHIFT_ANALYSIS.md)** - Temporal Shift 근본원인 분석 (Feature Distribution Shift 규명)
- **[results/PAPER_DRAFT_ELEMENTS.md](results/PAPER_DRAFT_ELEMENTS.md)** - 논문 핵심 요소 정리 (Contributions, Limitations, Figure/Table 목록)
- **[results/FINAL_REPORT.md](results/FINAL_REPORT.md)** - 최종 실험 결과 요약
- **[results/RESEARCH_REPORT_FINAL.md](results/RESEARCH_REPORT_FINAL.md)** - 최종 연구 보고서

### 논문용 Figure (9개)
`results/figures/` 디렉토리에 고해상도 PNG로 생성:
- Confusion Matrix, ROC Curve, PR Curve, Error 분포, 모델 상관 히트맵, Scaling Curve, Temporal Per-Category, Ensemble Ablation, Probe Depth Ablation
- 생성 스크립트: `python scripts/generate_paper_figures.py`

### 전체 실험 재현
```bash
bash scripts/run_full_experiment.sh  # 12단계 자동화, ~19시간 (GPU 1대)
```

**핵심 연구 질문 답변** (보완 실험 완료):
- **RQ1** (표현 품질): Foundation model features + 1-layer MLP로 95.42% F1 달성. True linear probe는 80.43%로 비선형 변환 필수. ResNet50-Frozen(78.42%) 대비 결정적 우위.
- **RQ2** (앙상블 강건성): **실패**. 앙상블(95.81%) < SigLIP 단독(96.13%). Temporal shift 시 모든 모델 28-35%p 하락, 앙상블도 예외 아님. 에러 상관 0.34-0.44.
- **RQ3** (Multi-task vs 독립): **부정적 전이**. 5차원 중 4차원에서 독립 학습이 multi-task보다 우수 (Fall: 94.24% vs 76.54%).

**상세 분석**: [docs/RESEARCH_AUDIT.md](docs/RESEARCH_AUDIT.md)
...
- **[TODO 및 개선사항](TODO_ko.md)** - 알려진 이슈, 계획된 기능, 코드-문서 정합성 개선 목록
